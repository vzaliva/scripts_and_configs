...
do
    ...
    if [[ $line =~ ^(HDMI-[0-9]+|DisplayPort-[0-9]+(-[0-9]+)?)\ connected ]]; then
...
if [ -z "$DP" ]; then
    ...
    i3-msg -q "workspace 7; move workspace to output eDP; workspace 8; move workspace to output eDP"
else
    if [ "$verbose" = true ]; then
        echo "Found external monitor $DP"
    fi
    xrandr --output $DP --auto
    xrandr --output $DP -s 3840x2160 --above eDP --rotate normal
    xrandr --dpi 96/eDP
